...
from shapes import Color, Line, Point, Shape
...
def find_line(p: Point, line: Line) -> Line:
    ...
    dot_product = (p.p1[0] - line.p1[0]) * (line.p2[0] - line.p1[0]) + (
        p.p1[1] - line.p1[1]
    ) * (line.p2[1] - line.p1[1])
...
def main() -> None:
    ...
    points: list[Shape] = [
        ...
        Point([45, 120]),
        Line([50, 100], [100, 50]),
        Line([58, 150], [200, 50], Color.LIGHT_BLUE),
    ]

    # TODO: Fix polymorhpism
    l = find_line(points[0], points[2])
    l.set_color(Color.BABY_BLUE)
    l.draw(frame)
